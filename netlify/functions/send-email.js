export async function handler(event) {
  ...
  try {
    ...
    // Compatibilidad: mantenemos las claves entrantes (`barberName`/`barberEmail`)
    const professionalName = barberName || '';
    const professionalEmail = barberEmail || '';
    ...
    const emailBody = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #fafafa;">
        <div style="background: linear-gradient(135deg, #1a1a1a 0%, #2c2c2c 100%); color: white; padding: 30px; border-radius: 12px 12px 0 0; text-align: center;">
          <h1 style="margin: 0; font-size: 28px;">Qita</h1>
        ...
        <div style="background: white; padding: 30px; border-radius: 0 0 12px 12px; box-shadow: 0 4px 16px rgba(0,0,0,0.1);">
          ...
          ${!autoAccepted ? `
            ...
            <div style="text-align: center; margin: 30px 0;">
              <a href="${process.env.URL || 'https://qita.netlify.app'}/dashboard" 
          ...
          <p style="color: #6b6b6b; font-size: 14px; text-align: center; margin: 0;">
            Este es un email automático de Qita. No respondas a este mensaje.
          </p>
        </div>
      </div>
    `;
    ...
    if (!emailApiKey) {
      console.warn('EMAIL_API_KEY not configured. Email would be sent to:', professionalEmail);
      return { 
        statusCode: 200, 
        body: JSON.stringify({ 
          success: true, 
          message: 'Email API not configured (development mode)',
          wouldSendTo: professionalEmail 
        }) 
      };
    }
    ...
    await sgMail.send({
      to: barberEmail,
      from: 'noreply@qita.com', // Debe ser un email verificado en SendGrid
    ...
    */
    ...
    if (emailService === 'resend') {
      const response = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          from: 'Qita <onboarding@resend.dev>', // Cambiar por tu dominio verificado
          to: [barberEmail],
          subject: emailSubject,
          html: emailBody
        })
    ...
    console.log('To:', professionalEmail);
